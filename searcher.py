...
class IndexMaker():
    ...
    def return_search(self,search_word,file):
        index_list = []
        index_list = self.index_by_word.get(search_word)
        if index_list != [] and index_list != None:
            print(file,"\n")
    ...
    def main(argv):
        ...
        search_word = input("Please enter the a single word to search:\n")
        if search_word == "***exit***":
            sys.exit('User Request to end the work')
        im = IndexMaker()
        for i in range(0, len(in_file_names)):
            ...
            im.make_index()
            im.return_search(search_word,in_file_names[i])
